...
class CartCubit extends Cubit<CartState> {
  ...
  Future addCart(Cart cart,context) async {
  ...
  bool loadProductById = false;
  Product? product;

  Future getProductById({id}) async {
    loadProductById = true;
    emit(GetProductByIdLoad());

    var request = http.MultipartRequest(
        'GET', Uri.parse(baseUrl + '/product/get-product-by-id'));
    request.fields.addAll({'id': id.toString()});

    http.StreamedResponse response = await request.send();
    print(response.statusCode.toString() + ">>>>> byId");
    if (response.statusCode == 200) {
